...
function checkFriendshipExists(userId, friendId, callback) {
    db.get(`SELECT id FROM friends WHERE user_id = ? AND friend_id = ?`, [userId, friendId], callback);
}
...
function removeFriendship(userId, friendId, callback) {
    db.run(`DELETE FROM friends WHERE user_id = ? AND friend_id = ?`, [userId, friendId], callback);
}
...
function recordMatchHistory(data, callback) {
    const { userId, opponentName, subject, mode, isWin, eloChange, createdAt } = data;
    db.run(
        `INSERT INTO match_history (user_id, opponent_name, subject, mode, is_win, elo_change, created_at)
         VALUES (?, ?, ?, ?, ?, ?, ?)`,
        [userId, opponentName, subject, mode, isWin ? 1 : 0, eloChange, createdAt],
        callback
    );
}
